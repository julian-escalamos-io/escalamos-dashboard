...
function PLTab({ er, historico, selectedERMonth, modelFilter }) {
  const currentER = useMemo(() => {
    ...
    return er.find(r => r.monthKey === selectedERMonth) || er[er.length - 1]
  }, [er, selectedERMonth])
...
function IngresosTab({ servicios, historico, modelFilter }) {
  ...
  // Últimas métricas del Histórico para NRR y Churn
  ...
  const cols = [
    ...
    { key: 'meses',    label: 'Meses',    width: 60, align: 'right', render: v => v || '—' },
    { key: 'monto',    label: '$ / mes',  width: 90, align: 'right', render: v => (
      <span style={{ color: GREEN, fontWeight: 700, fontSize: 13 }}>{fmt(v)}</span>
    )},
  ]
  ...
  return (
    <>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 24 }}>
        ...
        <div style={{ background: 'rgba(26,31,54,0.04)', border: '1px solid rgba(26,31,54,0.08)', borderRadius: 12, padding: '16px 20px' }}>
          ...
          <span style={{ fontSize: 28, fontWeight: 800, color: lastHistMonth?.nrr > 0 ? GREEN : DANGER, letterSpacing: -0.5 }}>
            {lastHistMonth?.nrr ? `${Math.round(lastHistMonth.nrr * 100) || Math.round(lastHistMonth.nrr)}` : '—'}
          </span>
        </div>
      </div>
      ...
      {modelos.map(modelo => {
        ...
        return (
          <div key={modelo} style={{ background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.07)', boxShadow: '0 2px 8px rgba(0,0,0,0.05)', borderRadius: 16, padding: '18px 20px', marginBottom: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
              <ModeloBadge value={modelo} />
              <span style={{ fontSize: 11, color: 'rgba(26,31,54,0.45)', fontWeight: 600 }}>{rows.length} servicios · {[...new Set(rows.map(r => r.idCliente))].length} clientes</span>
              <span style={{ marginLeft: 'auto', fontSize: 18, fontWeight: 800, color: c.color }}>{fmt(total)}/mes</span>
            </div>
            <DataTable rows={rows.sort((a, b) => b.monto - a.monto)} columns={cols} />
          </div>
        )
      })}
    </>
