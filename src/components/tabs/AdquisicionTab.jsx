  ...
  return (
    <div style={{ display: 'flex', gap: 20, alignItems: 'center' }}>
      <svg width={120} height={120} style={{ flexShrink: 0, overflow: 'visible' }}>
        ...
        <text x={cx} y={cy + 10} textAnchor="middle" fill="rgba(26,31,54,0.38)" fontSize="8" fontFamily="Montserrat, sans-serif">leads</text>
      </svg>
  ...
  return (
    <>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.6fr', gap: 16, marginBottom: 10 }}>
        ...
        <div style={{ background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 14, padding: 24 }}>
          <span style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: 2, color: 'rgba(26,31,54,0.5)', fontWeight: 700, marginBottom: 8, display: 'block' }}>Evolución de leads por mes</span>
          <MiniChart
...
function InsightsBlock({ cohort, channel }) {
  ...
  return (
    <div style={{ background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 14, padding: 24 }}>
      ...
      {!insights && !loading && (
        <p style={{ fontSize: 13, color: 'rgba(26,31,54,0.55)', marginTop: 12, fontWeight: 500 }}>Genera un análisis enfocado en {channelLabel.toLowerCase()} para el período seleccionado.</p>
      )}
      {insights && (
        <>
