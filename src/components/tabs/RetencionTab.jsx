...
export function RetencionTab() {
  return (
    <div style={{ maxWidth: 640, margin: '60px auto', textAlign: 'center' }}>
      ...
      <h2 style={{ fontSize: 18, fontWeight: 700, color: 'rgba(26,31,54,0.75)', margin: '0 0 8px' }}>Retención — Próximamente</h2>
      <p style={{ fontSize: 13, color: 'rgba(26,31,54,0.45)', fontWeight: 500, lineHeight: 1.6, marginBottom: 40 }}>
        Esta sección estará disponible cuando tengamos datos de recompra y LTV en el sistema.
      </p>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {COMING.map(({ icon, label, desc }) => (
          <div key={label} style={{ display: 'flex', gap: 16, alignItems: 'flex-start', background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 12, padding: '14px 18px', textAlign: 'left' }}>
            <span style={{ fontSize: 20, flexShrink: 0, opacity: 0.5 }}>{icon}</span>
            <div>
              <div style={{ fontSize: 13, fontWeight: 700, color: 'rgba(26,31,54,0.65)', marginBottom: 3 }}>{label}</div>
              <div style={{ fontSize: 12, color: 'rgba(26,31,54,0.38)', lineHeight: 1.5, fontWeight: 500 }}>{desc}</div>
            </div>
          </div>
        ))}
      </div>
