...
// Xero Raw Data columns: A(0):Fecha C(2):Modelo D(3):Tipo E(4):AccountCode
//   G(6):ContactName K(10):MontoUSD N(13):Status O(14):FechaPago
...
export function computeCollectionPace(xeroRaw, currentRow, prevRow) {
  ...
  // Filter: revenue items with payment date (account code starts with 2, monto > 0, has fecha pago)
  const revenueItems = xeroRaw.filter(r => {
    const acCode = String(r[4] || '').trim()
    const tipo = String(r[3] || '')
    const monto = +r[10] || 0
...
export function computeRecentChurn(servicios, modelFilter, dateRange) {
  // Si no hay rango, default = últimos 3 meses
  let startKey, endKey
  if (dateRange?.start && dateRange?.end) {
    startKey = `${dateRange.start.getFullYear()}-${String(dateRange.start.getMonth() + 1).padStart(2, '0')}`
    endKey = `${dateRange.end.getFullYear()}-${String(dateRange.end.getMonth() + 1).padStart(2, '0')}`
  } else {
    const now = new Date()
    startKey = new Date(now.getFullYear(), now.getMonth() - 2, 1).toISOString().slice(0, 7)
